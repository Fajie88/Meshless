...
%         iexample=9: u_exact=cos(x).*cosh(y)+sin(x).*sinh(y)+exp(x).*cos(y)+exp(y).*sin(x)+x.^2-y.^2+2*x+3*y+1
...
%         iexample=5: u_exact=cos(0.5*x+(lamda^2-0.5^2)^(1/2)*y);  % Helmholtz: Computers and Structures 83 (2005) 267¨C278: example 2
...
if strcmp(type_eq,'Laplace')
    ...
    elseif iexample==9
        ...
        ux_exact = @(x,y) 2*x + exp(x).*cos(y) + exp(y).*cos(x) + cos(x).*sinh(y) - cosh(y).*sin(x) + 2;
        uy_exact = @(x,y) cos(x).*sinh(y) - 2*y + cosh(y).*sin(x) - exp(x).*sin(y) + exp(y).*sin(x) + 3;
    end
...
elseif strcmp(type_eq,'Helmholtz')
    ...
    elseif iexample==5 
        u_exact = @(x,y) cos(0.5*x+(lamda^2-0.5^2).^(1/2)*y);
        ux_exact = @(x,y) -sin(x/2 + y*(lamda^2 - 1/4).^(1/2))/2;
        uy_exact = @(x,y) -sin(x/2 + y*(lamda^2 - 1/4).^(1/2))*(lamda^2 - 1/4)^(1/2); 
...
elseif strcmp(type_eq,'ModifiedHelmholtz')
    ...
    elseif iexample==3  %modified Helmholtz equation
        u_exact = @(x,y) exp((lamda/2)*(x+3^(1/2)*y));
        ux_exact = @(x,y) (lamda*exp((lamda*(x + 3^(1/2)*y))/2))/2;
        uy_exact = @(x,y) (3^(1/2)*lamda*exp((lamda*(x + 3^(1/2)*y))/2))/2;
